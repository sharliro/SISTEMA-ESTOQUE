  ...
  return (
    <div className="page">
      <header className="page-header">
        <div>
          <h1>{activeTab === 'entry' ? 'Entradas' : 'Saídas'}</h1>
          <p>{activeTab === 'entry' ? 'Adicione itens ao estoque informando quantidade, NFe, unidade e setor.' : 'Registre a remoção de itens indicando produto, quantidade, unidade e setor.'}</p>
        </div>
      </header>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (/^\d{4}-\d{2}-\d{2}$/.test(sanitized)) {
                        ...
                        if (m < 1 || m > 12 || d < 1 || d > 31) { setDtNfeError('Data inválida'); setEntryForm({ ...entryForm, dtNfe: prev }); if (dtNfeRef.current) dtNfeRef.current.value = prev || ''; return; }
